...
```
收藏 Favorites         本机/U盘 Local        NAS 媒体库 DMS        当前队列 Queue
├─ 我喜欢的             ├─ 歌手                ├─ <每台 NAS/DLNA 服务器>  （点任意一首直接跳播）
├─ 电台                 ├─ 专辑                │    └─ 文件夹 → 曲目
├─ 歌单 → 曲目          ├─ 文件夹
└─ 榜单 → 曲目          └─ 全部歌曲
```

- 点任意**歌单/分类/NAS 文件夹**上的播放键 = 整列表替换队列并开播；点**单曲** =
  从那首开始播；
- 全程不需要在 HA 里登录任何音乐账号——浏览的是音箱本地数据，取流由音箱自己完成；
- **NAS/DMS**：局域网里任何 DLNA 媒体服务器（QNAP/群晖/minidlna/Plex…）自动出现
  在「NAS 媒体库」下，点歌即推给音箱直链播放（songSrc=21），音箱上会显示来源
  NAS 的名字；无需在 HA 配置任何东西；
